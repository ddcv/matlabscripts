...
for y = 1:100
        ...
        for w = 1:50                                            % Histogram counts for Problem 4b.
            edge1 = (w-1)*20 + 1;                               %
            edge2 = (w-1)*20 + 20;                              %
            count(w,y) = sum(spikes(y,edge1:edge2));            %
        end
        
end

timeISI = linspace(0,T,50);

countsum = sum(count,2)/100;
bar(timeISI,countsum) %

%% TODO
% Plot the expected firing rate defined by equations on the same plot as
% above. Do the two agree?
