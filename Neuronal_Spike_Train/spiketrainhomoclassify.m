% Simulated Data Hwk 3 problem 6a ELEC 548
% By Dan Calderon, 10/16/12
%
%
%
%
...
%counttrial = zeros(length(k),100,16);              %Accomodating per neuron array
%countmean  = zeros(1,length(k),16);                %Accomodating per neuron array
...
for i = 1:length(k)
    for y = 1:100
        ...
        counttrial(i,y,neuron) = sum(spikes(i,:));                     % Key contribution for Problem 3c.
        countmean(1,i,neuron) = sum(counttrial(i,:,neuron),2)/100;
...
for l = 1:100
    ...
    hold on
    scatter(k,counttrial(:,l,6),[], color)
    scatter(k,countmean(1,:,6),[], [1 0 0])
    xlabel('k')
    ylabel('Spike Count')
end
