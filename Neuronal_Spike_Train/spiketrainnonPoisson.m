...
% reaching angle associated with the maximum response rmax, and r0 is an offset that shifts
...
figure()
for y = 1:100 
        lambda(1) = r0 + (rmax - r0) * (1+cos(-pi*time(1)))/2 * unit(time(1)-tau);
        ISI(1) = exprnd(lambda(1))/1000;                % Generate 1000 ISIs  

        for j = 2:1000                                          % time loop 
            s(j) = time(j) * pi;                                % Specified angles for Problem
            lambda(j) = r0 + (rmax - r0) * (1+cos(-pi*time(j)))/2 * unit(time(j)-time(j-1)-tau);        % Lambda Equation 1.15 from Dayan & Abbott
